...
public class DateUtil {
    ...
    public static final String DATEMONTHCZDAY = "yyyy年MM月dd日";
     ...
     * @return
     */
    public static int getMaxDays(String dateStr){
    ...
    /**
     * 计算times开始经过 毫秒 后的日期时间
     * @param startDate
     * @param milliseconds   秒数 不是毫秒
     * @return
     */
    public static String getDateAfterMilliseconds(String startDate, long milliseconds){
        try {
            SimpleDateFormat format = new SimpleDateFormat(DATETIME);
            return format.format(toDateTime(startDate).getTime() + milliseconds);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static String getDateAfterTime(String startDate, long seconds){
        return getDateAfterMilliseconds(startDate, seconds * 1000);
    }
