...
import com.beyond.library.view.pullrefresh.internal.IndicatorLayout;
...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
     ...
     * 最小的出发时间戳
     */
    private final long MIN_TIME_STAMP = 100;
    /**
     * 默认倒数第1个
     */
    private final int DEFAULT_RECIPROCAL_COUNT = 1;
    /**
     * 倒数第几个
     */
    private int reciprocalCount = DEFAULT_RECIPROCAL_COUNT;
    /**
     * 最近一次的可视item position
     */
    ...
    public void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount,
        ...
        if (null != mOnLastItemVisibleListener) {
            mLastItemVisible = (totalItemCount > 0) && (firstVisibleItem + visibleItemCount >= totalItemCount - DEFAULT_RECIPROCAL_COUNT);
        }
        ...
        if (totalItemCount > 0 && Math.abs(firstVisibleItem - lastFirstVisibleItem) > 1) {
            int targetPosition = (totalItemCount - reciprocalCount);
            if (firstVisibleItem > lastFirstVisibleItem) {  //向上
//				L.v("targetPosition==========> targetPosition: "+targetPosition+" totalItemCount: "+totalItemCount+"  firstVisibleItem: "+firstVisibleItem);
                if (targetPosition >= firstVisibleItem && targetPosition <= (firstVisibleItem + visibleItemCount)) {
                    long currTimeStamp = System.currentTimeMillis();
                    if ((currTimeStamp - lastTimeStamp) > MIN_TIME_STAMP) {
                        lastTimeStamp = currTimeStamp;
                        if (mOnLastItemVisibleListener != null && !isLoadingMore && !isLoadMoreCompleteAndNoData()) {
                            isLoadingMore = true;
                            mOnLastItemVisibleListener.onLastItemVisible();
                        }
                    }
                }
            } else if (firstVisibleItem < lastFirstVisibleItem) {    //向下
     ...
     * getRefreshableView()}.
     * {@link android.widget.AdapterView#setAdapter(Adapter)}
     ...
     * Pass-through method for {@link PullToRefreshBase#getRefreshableView()
     * getRefreshableView()}.
     * {@link android.widget.AdapterView#setOnItemClickListener(OnItemClickListener)
    ...
    private void addIndicatorViews() {
        ...
        if (mode.showHeaderLoadingLayout() && null == mIndicatorIvTop) {
            ...
            // If we can't pull down, but have a View then remove itD
    ...
    /**
     * 设置显示倒数第几个时 回调 OnLastItemVisibleListener
     *
     * @param reciprocalCount
     */
    public void setReciprocalCount(int reciprocalCount) {
        if (reciprocalCount <= 0) {
            reciprocalCount = DEFAULT_RECIPROCAL_COUNT;
        }
        this.reciprocalCount = reciprocalCount;
    }
