...
import com.beyond.library.view.pullrefresh.internal.FlipLoadingLayout;
import com.beyond.library.view.pullrefresh.internal.FrameLoadingLayout;
import com.beyond.library.view.pullrefresh.internal.LoadingLayout;
import com.beyond.library.view.pullrefresh.internal.RotateLoadingLayout;
import com.beyond.library.view.pullrefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public static final int SMOOTH_SCROLL_LONG_DURATION_MS = 200;
    static final int DEMO_SCROLL_INTERVAL = 200;
    ...
    /**
     * mHeaderLayout下拉时总是显示
     */
    private boolean mHeaderLayoutAlwaysVisibilityWhenPull = true;
...
//	private AnimationStyle mLoadingAnimationStyle = AnimationStyle.getDefault();
    ...
    /**
     * LoadMoreComplete 被调用时的 原始的 mode
     */
    private Mode originModeLoadMoreComplete;
    ...
    /**
     * 加载更多结束且没有更多数据  显示 萌主到底了
     */
    public void onLoadMoreCompleteAndNoData() {
        if (mMode == Mode.PULL_FROM_START || mMode == Mode.BOTH || mMode == Mode.PULL_FROM_END) {
            if (originModeLoadMoreComplete == null) {
                originModeLoadMoreComplete = mMode;
            }
            if (mMode == Mode.PULL_FROM_START) {
                setMode(Mode.BOTH);
            }
            if (mFooterLayout != null) {
                mFooterLayout.showFooterNoDataView();
            }
        }
    }

    /**
     * 加载更多结束且还有更多数据  隐藏 萌主到底了
     */
    public void onLoadMoreComplete() {
        if (originModeLoadMoreComplete != null) {
            setMode(originModeLoadMoreComplete);
        }
        if (mFooterLayout != null) {
            mFooterLayout.hiddenFooterNoDataView();
        }
    }

    /**
     * 是否已经 加载更多结束且没有更多数据  显示 萌主到底了
     * @return
     */
    public boolean isLoadMoreCompleteAndNoData() {
        if (mFooterLayout != null) {
            return mFooterLayout.isShowFooterNoDataView();
        }
        return false;
    }
    ...
    /**
     * 是否下拉刷新	此方法只在 Mode.BOTH
     * @return
     */
    public final boolean isPullDownToRefresh() {
     ...
     * 设置成下拉刷新状态
     */
    public final void setTopRefreshing() {
    ...
    /**
     * mHeaderLayout 下拉时总是显示
     */
    public final void disableHeaderLayoutAlwaysVisibilityWhenPull() {
        mHeaderLayoutAlwaysVisibilityWhenPull = false;
    }
    ...
    protected void onRefreshing(final boolean doScroll) {
        if (mCurrentMode == Mode.PULL_FROM_START && mMode.showHeaderLoadingLayout()) {
            mHeaderLayout.refreshing();
        }
        if (mCurrentMode == Mode.PULL_FROM_END && mMode.showFooterLoadingLayout()) {
            if (mFooterLayout.isShowFooterNoDataView()) {    //显示 到底了 直接还原上拉效果
                setState(State.RESET);
                return;
            } else {
        ...
        if (doScroll) {
            if (mCurrentMode == Mode.PULL_FROM_START && !mHeaderLayoutAlwaysVisibilityWhenPull) { //下拉刷新  mHeaderLayoutAlwaysVisibilityWhenPull:mHeaderLayout下拉时总是显示
    ...
    protected final void setHeaderScroll(int value) {
        ...
        pullHeaderToZoom(value);
        if (onPullZoomListener != null) {
            onPullZoomListener.onPullZooming(value);
        }
        ...
        if (mLayoutVisibilityChangesEnabled) {
            if (value < 0) {
                if (mHeaderLayoutAlwaysVisibilityWhenPull) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrRefreshableViewBackground)) {
            ...
            Log.w(LOG_TAG, "You're using the deprecated ptrAdapterViewBackground attr, please switch over to ptrRefreshableViewBackground");
    ...
    private void pullEvent() {
        ...
        if (mCurrentMode == Mode.PULL_FROM_START && mMode.showHeaderLoadingLayout() && !mHeaderLayout.isUserIntrinsicAnimRunning() && mHeaderLayout.isShowAnimWhenPull()) {
            mHeaderLayout.refreshing();
        }
        if (mCurrentMode == Mode.PULL_FROM_END && mMode.showFooterLoadingLayout() && !mFooterLayout.isUserIntrinsicAnimRunning() && mFooterLayout.isShowAnimWhenPull()) {
            mFooterLayout.refreshing();
        }
    ...
    public enum AnimationStyle {
         ...
         * @return
         */
        static AnimationStyle getFrameAnim() {
            return FRAME;
        }
