 ...
 */
public class SocialCommentDetailTwoActivity extends BaseActivity {
    ...
    public static void startActivityForResult(Activity context, String articleId, int requestCode){
        Intent intent = new Intent(context, SocialCommentDetailTwoActivity.class);
        intent.putExtra(EXTRA_ARTICLE_INFO_KEY, articleId);
...
//        ToastUtil.showCenter(SocialCommentDetailActivity.this,"恭喜您, + 1 科普绿币!");
        articleId =  getIntent().getStringExtra(EXTRA_ARTICLE_INFO_KEY);
        if(articleId == null){
            backNoAnim();
            return ;
        }
    ...
    private void addHeaderView(){
        ...
        socialCircleContentListRecyclerAdapter.getDataList().add(new ArticleInfo());
        ...
        publishedCommentView.setOnPublishedOnClickListener(new PublishedCommentView.SimplePublishedClickListener() {
            ...
            public void onOk() {
                if(TextUtils.isEmpty(publishedCommentView.getText().trim())){
                    ToastUtil.showCenter(SocialCommentDetailTwoActivity.this, "请输入评论");
                    return ;
                }
    ...
    private void requestPublishedComment(){
        showProgressDialog();
        socialRestUsage.sentArticleComment(REQUEST_ARTICLE_COMMENT_TASK_ID, articleId, publishedCommentView.getText());
    }
    ...
    private void requestComment(){
        socialRestUsage.articleDetail(REQUEST_ARTICLE_DETAIL_TASK_ID, articleId);
    }
