...
public class AllOrderAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public List<OrderlsitBean> list;
    public Context context;
    private BottomButtonClickLinster linster;
    private ItemOnClickLinster itemLinster;


    public AllOrderAdapter(List<OrderlsitBean> list, Context context, int position) {
        this.list = list;
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof OneViewHolder) {
            ...
            ((OneViewHolder) holder).tvPirce.setText("应收：￥" + list.get(position).getPayAmount() + "");
            if (tabPosition == 0) {
                //这个是未付款
                ...
                ((OneViewHolder) holder).tvBtn1.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        linster.onClick(2, list.get(position).getOrderSn());
                    }
                });
            }
            if (tabPosition == 1) {
                ((OneViewHolder) holder).tvBottomBtn.setVisibility(View.VISIBLE);
                ((OneViewHolder) holder).tvBtn1.setText("拒绝订单");
                ...
                ((OneViewHolder) holder).tvBtn2.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        linster.onClick(1, list.get(position).getOrderSn());
                    }
                });
            ...
            if (tabPosition == 2) {
                ...
                ((OneViewHolder) holder).tvBtn2.setText("查看物流");
                ((OneViewHolder) holder).tvBtn2.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        linster.onClick(3, list.get(position).getOrderSn());
                    }
                });
