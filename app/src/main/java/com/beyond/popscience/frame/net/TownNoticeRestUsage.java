...
public class TownNoticeRestUsage extends AppBaseRestUsageV2 {
    ...
    public void getNews(int taskId, String villageId, int page){
        String url = String.format(GET_NEWS_two, villageId == null ? "" : villageId,  UserInfoUtil.getInstance().getUserInfo().getUserId(),page);
    ...
    public void getNewsDetail(int taskId, String newsId){
       // get(String.format(GET_NEWS_DETAIL, newsId == null ? "" : newsId), null, new NewCustomResponseHandler<NewsDetailObj>(taskId){});
        get(String.format(GET_NEWS_DETAIL_two, newsId == null ? "" : newsId), null, new NewCustomResponseHandler<NewsDetailObj>(taskId){});
    }
