package com.beyond.popscience.widget.draggridview;
...
public class DragGridView extends GridView{
	 ...
	 */
	public static long dragResponseMS = 700;
    ...
    private int canDragIndex = 0;//从第几个开始可以拖拽
...
//			mVibrator.vibrate(50); //震动一下
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if(isDrag && mDragImageView != null&& mDragPosition>=canDragIndex){
			switch(ev.getAction()){
				case MotionEvent.ACTION_MOVE:
					moveX = (int) ev.getX();
					moveY = (int) ev.getY();
...
//		mWindowLayoutParams.alpha = 0.55f; //透明度
	...
	private void onSwapItem(int moveX, int moveY){
        ...
        if (tempPosition <canDragIndex) return;
     ...
     * @param position
     */
	public void setCanDragIndex(int position){
        if (position>getCount()){
            position = 0;
        }
        this.canDragIndex = position;
    }
